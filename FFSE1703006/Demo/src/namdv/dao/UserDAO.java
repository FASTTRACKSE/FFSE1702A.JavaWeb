...
public class UserDAO {
	...
	public static int countRecords() {
		try {
			PreparedStatement ps = con.prepareStatement("select count(*) from testcrud");
	...
	public static List<User> getRecords(int start, int total) {
		List<User> list = new ArrayList<User>();
		try {
			PreparedStatement ps = con.prepareStatement("select * from testcrud limit " + (start - 1) + "," + total);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				User u = new User();
				u.setId(rs.getString("id"));
